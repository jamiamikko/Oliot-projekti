...
/**
 * Tama on luokka DetectedObjectListener, joka huolehtii esteiden
 * havaitsemisesta ja vaistamisesta. Luokka toteuttaa FeatureListener
 * rajapinnan, joka ilmoittaa etaisyyksia ja kulmia.
 * 
 * @author jamiamikko
 *
 */
...
public class DetectedObjectListener implements FeatureListener {
	...
	public void featureDetected(final Feature feature, final FeatureDetector detector) {
		...
		if (range <= 30) {
			...
			// Jos sade on pienempi kuin 3 mm, ohjelma pysaytetaan kokonaan.

			if (range <= 3) {
				System.out.println("Too close! Aborting!");
				System.exit(0);
			}
